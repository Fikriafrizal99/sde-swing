...
def style_sheet(ws, table: bool = True) -> None:
    ...
    for header, col in headers.items():
        ...
        if any(x in upper for x in ("RETURN_PCT", "EXPECTANCY_PCT", "WIN_RATE_PCT", "PROFIT_FACTOR")) and ws.max_row >= 2:
            ws.conditional_formatting.add(
                ...
                ColorScaleRule(start_type="min", start_color="FECACA", mid_type="percentile", mid_value=50,
                               mid_color="FEF3C7", end_type="max", end_color="BBF7D0"),
            )
    ...
    if table and ws.max_row >= 2:
        name = re.sub(r"[^A-Za-z0-9_]", "_", f"tbl_{ws.title}")[:200]
