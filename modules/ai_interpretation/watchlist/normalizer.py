...
def build_presentation_context(context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _put(broker, "status raw PRIMARY", _human_enum(facts.get("primary_raw_status")))
    _put(broker, "data broker hari ini", _human_enum(facts.get("today_pulse_status")))
    _put(broker, "net flow hari ini", _money(facts.get("today_pulse_net_flow")))
    _put(broker, "arah hari ini", _human_enum(facts.get("today_pulse_direction") or facts.get("today_pulse_broker_state")))
    _put(broker, "konsentrasi buyer hari ini", _percent(facts.get("today_pulse_buyer_concentration"), ratio=True))
    _put(broker, "konsentrasi seller hari ini", _percent(facts.get("today_pulse_seller_concentration"), ratio=True))
